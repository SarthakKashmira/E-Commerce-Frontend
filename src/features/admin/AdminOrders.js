...
import { fetchAllOrdersAsync, fetchAllOrdersByFilterAsync, selectOrders, selectTotalOrders, updateOrderAsync } from '../order/orderSlice';
...
function AdminOrders() {
  ...
  useEffect(() => {
    ...
    dispatch(fetchAllOrdersByFilterAsync({sort,pagination}));
    // dispatch(fetchAllOrdersAsync({ _page: 2, _limit: 2 }));
  }, [dispatch, page,sort]);
                ...
                {orders && orders.map((order,index) => (
                  ...
                  >
                    ...
                    <td className="py-3 px-0 text-center">
                      <div className="">
                        <div>
                          <strong>{order.selectedAddress.firstName} {order.selectedAddress.lastName}</strong>,
                        </div>
                        <div>{order.selectedAddress.streetAddress},</div>
                        <div>{order.selectedAddress.city}, </div>
                        <div>{order.selectedAddress.state}, </div>
                        <div>{order.selectedAddress.postalCode}, </div>
                        <div>{order.selectedAddress.phoneNumber}, </div>
                      </div>
                    </td>
                    <td className="py-3 px-0 text-center">
